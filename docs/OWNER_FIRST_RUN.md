...
| Line begins with | Meaning | What to do |
|---|---|---|
| `unreal.<Class> is not available` | The C++ classes are not compiled into this editor. | Finish step 2, restart the editor, re-run the script. |
...
| `Set IA_... Value Type to BOOLEAN` | The action asset exists but its value type could not be set. | Open the asset, set **Value Type = Digital (bool)**, save. |
...
| `Project Settings > Game > Ascension: set ...` / `Config/DefaultGame.ini already carries ...` | The settings CDO could not be written or saved from Python. | Nothing is lost: `Config/DefaultGame.ini` already holds the four soft references. Open **Project Settings → Game → Ascension** and confirm Ladder Config, Ladder Table, Debug Overlay Class and Debug Panel Class are filled. |
...
3. Press **F2**. `IA_DebugPanel` is mapped in both contexts; the player controller creates `WBP_DebugPanel` from **Project Settings → Game → Ascension → Debug Panel Class**. Screenshot the panel → `docs/evidence/M0_debug_panel.png`.
...
`docs/MILESTONE_0_REPORT.md` follows the charter 14.1 format and already says what was and was not verified in the container. Replace the placeholders:
